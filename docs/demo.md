...
- A rehearsed one-line answer ready for "why not just use Otter.ai/Fireflies" (`project-context.md`'s pitch, restated: hardware-based attribution instead of guessing, fully offline, queryable memory).
...
7. **Close with the offline claim.** Mention (or, if time allows, show) that this all just happened with no internet connection — reinforces the differentiation one more time before questions.
...
|---|---|
| "Why not just use Otter/Fireflies?" | They guess speaker identity from one mixed mic; we get it from which phone the audio came from, which is inherently more reliable, and we run fully offline with zero per-meeting cost. |
