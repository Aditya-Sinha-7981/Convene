...
**Implementation status:** this is a contract for code that does not exist yet. The DT-17 prototype exposes only `GET /join/{meeting}`, `GET /app.js`, `GET /ws/{meeting}` and `GET /metrics`. Nothing below is a claim that a route works.
