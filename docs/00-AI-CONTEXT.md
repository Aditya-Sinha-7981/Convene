...
**Implementation status:** The checked-in system implements the demo path through CON-09: WebRTC transport, meeting/device registry, local STT, dedicated-device attribution, manual correction, the live dashboard, transcript chunking with local embeddings, and grounded live Q&A with a local LLM. Shared-device enrollment/classification, summary, DOCX export, and history remain planned. Real-phone, Wi-Fi, and offline behavior are not yet validated; see [manual-tests.md](manual-tests.md). See [documentation index](README.md) for navigation and [implementation plan](implementation-plan.md) for the build sequence.
