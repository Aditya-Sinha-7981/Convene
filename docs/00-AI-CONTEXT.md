...
**Implementation status:** The checked-in system implements the demo path through CON-08: WebRTC transport, meeting/device registry, local STT, dedicated-device attribution, manual correction, the live dashboard, and transcript chunking with local embeddings (the index live Q&A will search). Shared-device enrollment/classification, live Q&A, summary, DOCX export, and history remain planned. Real-phone, Wi-Fi, and offline behavior are not yet validated; see [manual-tests.md](manual-tests.md). See [documentation index](README.md) for navigation and [implementation plan](implementation-plan.md) for the build sequence.
